...
/**
 * Binary CSR snapshot of the movie graph. The neighbor section (2 ints per edge, by far
 * the dominant part) is memory-mapped rather than loaded, so the graph is served from the
 * OS page cache instead of the Java heap; only the per-node arrays are materialized.
 * A DB rebuild scatter-fills neighbors directly into the mapped tmp file via {@link Builder},
 * so peak heap stays independent of edge count.
 */
public final class GraphSnapshot {
    ...
    public static final class Builder {
        ...
        private Builder(Path target, long[] ids, int[] offsets) throws IOException {
            ...
            meta.putInt(MAGIC).putInt(VERSION).putInt(n);
